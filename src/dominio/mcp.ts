...
// de banco, sem import do SDK: dá para testar tudo aqui sem conexão e sem
...
//    mensagem de erro que ensina o modelo a acertar na próxima tentativa.
...
//    pelo mesmo motivo de src/dominio/prompt.ts: esta é uma rota de saída sem
//    volta.
//
// Também é aqui que mora o controle de tamanho (limite de rodadas, teto de
// sugestões, truncagem de conteúdo de contexto). Contexto cheio de coisa
 ...
 * lista a `Tool[]` e o TypeScript confere a compatibilidade estrutural lá.
 */
...
/** Teto de rodadas devolvidas por `ver_rodadas` quando o chamador não pede outro. */
export const LIMITE_RODADAS_PADRAO = 5;
/** Teto absoluto de `ver_rodadas` — controle de custo, não de banco. */
...
export const FERRAMENTAS_MCP: readonly FerramentaMcp[] = [
  {
    ...
    description:
      ...
      "Use quando precisar saber quais projetos existem, ou para descobrir o nome exato de um " +
      "projeto antes de chamar qualquer outra ferramenta daqui. " +
  ...
  },
  {
    name: "ver_rodadas",
    description:
      "Histórico de diagnóstico de um projeto: o que cada rodada noturna encontrou, o status, " +
      "se os testes passaram e o achado de cada agente que rodou. Mais recente primeiro. " +
      ...
      "A rodada só diagnostica: nada do que aparece aqui foi executado.",
    inputSchema: {
      type: "object",
      properties: {
        ...
        limite: {
          type: "integer",
          description: `Quantas rodadas trazer, da mais recente para trás. Padrão ${LIMITE_RODADAS_PADRAO}, máximo ${LIMITE_RODADAS_MAXIMO}.`,
          minimum: 1,
  ...
  },
  {
    ...
    description:
      ...
      "ferramenta para isso aqui, e você não deve tentar contorná-la por outro caminho.",
    inputSchema: {
      type: "object",
      properties: {
        projeto: PROPRIEDADE_PROJETO,
  ...
  },
  {
    ...
    description:
      ...
      "Use quando precisar saber que orientação os agentes daquele projeto já recebem, e sempre antes " +
      "de chamar anexar_contexto, para não propor algo que já está lá. " +
  ...
  },
  {
    ...
    description:
      ...
      "Confirme o repositório com ele antes de chamar: não existe ferramenta para apagar projeto daqui. " +
      "Cadastrar aqui não faz o repositório entrar sozinho na routine noturna; a lista de repositórios " +
      "dela é mantida à parte e o dono precisa somar o repositório lá também.",
    inputSchema: {
      type: "object",
      properties: {
        ...
        repositorio: {
          type: "string",
          description: 'Repositório no formato "dono/repo", sem URL e sem espaços.',
          maxLength: 140,
        },
        frequencia: {
          type: "string",
    ...
    annotations: { title: "Cadastrar projeto", readOnlyHint: false, destructiveHint: false, idempotentHint: false },
  },
  {
    ...
    description:
      ...
      "daquele projeto vai lê-lo, sem ninguém acordado para conferir. " +
      ...
      "Se já existir contexto com o mesmo agente e o mesmo tipo, a ferramenta recusa e mostra o que está " +
      "lá; só repita com substituir=true depois de o dono confirmar que quer trocar.",
    inputSchema: {
      type: "object",
      properties: {
        projeto: PROPRIEDADE_PROJETO,
...
/** Nomes das ferramentas, para o despacho conferir antes de executar. */
...
/**
 * Minúsculas, sem acento, sem espaço nas pontas — para comparar "Gravidade
 * Zero" com "gravidade zero". O intervalo `\u0300-\u036f` é o bloco de
 * diacríticos combinantes que o `NFD` separa das letras (escrito em escape,
 * não em caractere literal, para não depender de o arquivo chegar íntegro em
 * todo editor).
 */
 ...
 * exato, repositório exato (inteiro ou só a parte depois da barra), e por fim
 * pedaço de nome ou de repositório. Para no primeiro degrau que casar — assim
 * um projeto chamado "painel" não fica ambíguo só porque existe outro chamado
 * "painel-antigo".
 *
 ...
 * décima escreve contexto no projeto errado.
 */
...
/**
 * Normaliza o `limite` que veio da chamada da ferramenta. Valor ausente,
 * fracionário, negativo ou absurdo cai no padrão em vez de virar erro — o
 * modelo errar o tipo de um parâmetro opcional não vale interromper a
 * conversa.
 */
...
export interface SugestaoMcp {
  ...
  criada_em: string;
  /** Só presente quando `reversibilidade` é `nao_reverte` — o campo que o painel destaca antes de aprovar. */
  aviso?: string;
}
 ...
 * a fila de todos os projetos, e "qual projeto" é a primeira coisa que o dono
 * pergunta ao ver a lista.
 */
 ...
 * ferramenta de desfazer. Esta função encontra a colisão antes da escrita,
 * para `anexar_contexto` poder recusar e mostrar o que já existe em vez de
 * apagar em silêncio.
 *
...
export function contextoExistente(
  ...
  const agente = normalizar(agenteDestino);
  const rotulo = normalizar(tipo);
  return (
    contextos.find((c) => normalizar(c.agente_destino) === agente && normalizar(c.tipo) === rotulo) ?? null
  );
}
