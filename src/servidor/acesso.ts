 ...
 *
 * `mcp` existe por um motivo específico, e não por gosto de granularidade: o
 * Claude Code precisa mandar o header de bypass de qualquer forma, porque é
 * ele que passa pelo Vercel Authentication na borda. Se o MCP se apoiasse
 * só nisso, "a routine às 3h" e "o dono conversando no terminal" virariam a
 * mesma origem — e aí toda decisão futura do tipo "a routine não pode fazer
 * X" passaria a proibir o dono junto, ou pior, relaxar algo para o dono
 * abriria a mesma porta para a rodada sem supervisão. O segredo separado é o
 * que mantém as duas distinguíveis.
 *
 ...
 * construção. Por isso o MCP não expõe aprovar, recusar nem apagar — a defesa
 * ali é a ausência da ferramenta, não a credencial.
 */
...
async function resolverOrigemAcesso(): Promise<OrigemAcesso | null> {
  ...
  // O segredo do MCP vem antes do bypass de propósito: o Claude Code manda os
  // dois headers (o de bypass para atravessar a borda da Vercel, o do MCP para
  // se identificar), e a origem certa nesse caso é a mais específica.
